...
{
    public class SoportePagoUpdateEventHandler : IRequestHandler<SoportePagoUpdateCommand, int>
    {
        private readonly ApplicationDbContext _context;

        public SoportePagoUpdateEventHandler(ApplicationDbContext context)
        {
            _context = context;
        }

        public async Task<int> Handle(SoportePagoUpdateCommand request, CancellationToken cancellationToken)
        {
            try
            {
        ...
        private async Task<int> RealizaCargaMasiva(string path, string file, string usuarioId, int anio, string mes)
        {
            try
            {
                ...
                cmd.CommandText = "[Mensajeria].[sp_actualizaSoportePago]";
        ...
        private async Task<string> Copia(SoportePagoUpdateCommand request)
        {
            try
            {
        ...
        public bool eliminaArchivo(SoportePagoUpdateCommand request)
        {
            string newPath = Directory.GetCurrentDirectory() + "\\Soportes de Pago\\" + request.Anio + "\\" + request.Mes;
